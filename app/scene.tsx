...
import {SYSTEMS,regionOf,type Atlas,type SceneState,type Region} from './anatomy';
import {t,type Lang} from './i18n';
import {plName} from './pl-anatomy';
interface Props {atlas:Atlas;state:SceneState;lang:Lang;onSelect:(id:string)=>void;onProgress:(n:number)=>void;onError:(s:string)=>void;onZoom?:(z:number)=>void}
export default function AnatomyScene({atlas,state,lang,onSelect,onProgress,onError,onZoom}:Props){
 const host=useRef<HTMLDivElement>(null),latest=useRef(state),select=useRef(onSelect);
 latest.current=state;select.current=onSelect;
 const apiRef=useRef<{dolly?:(f:number)=>void;frame?:(r:string)=>void}>({});
 const zoomCb=useRef(onZoom);zoomCb.current=onZoom;
 useEffect(()=>{
  const el=host.current!;let disposed=false,frame=0,dirty=true,ready=false,lastView='',lastReset=-1,lastIsolate='',layoutKey='',amount=0;
  let lastState:SceneState|null=null;
  const abort=new AbortController();
  let renderer:T.WebGLRenderer;
  try{renderer=new T.WebGLRenderer({antialias:true,alpha:false,powerPreference:'high-performance'});}catch{onError(t('sceneErrorWebgl',lang));return;}
  renderer.setPixelRatio(Math.min(devicePixelRatio,innerWidth<768?1.5:2));renderer.setClearColor('#eef0fb');renderer.outputColorSpace=T.SRGBColorSpace;renderer.toneMapping=T.ACESFilmicToneMapping;renderer.toneMappingExposure=.92;el.appendChild(renderer.domElement);
  ...
  const hemi=new T.HemisphereLight(0xffffff,0xa7acb2,.75);scene.add(hemi);
  const key=new T.DirectionalLight(0xfff6ee,1.15);key.position.set(-2,4,3);scene.add(key);
  const rim=new T.DirectionalLight(0xe9f0ff,.85);rim.position.set(2,2,-3);scene.add(rim);
  const fill=new T.DirectionalLight(0xf2f4ff,.4);fill.position.set(2.5,1,2.5);scene.add(fill);
  const ground=new T.Mesh(new T.CircleGeometry(30,96),new T.MeshStandardMaterial({color:0xc9d0e8,roughness:1}));ground.rotation.x=-Math.PI/2;ground.position.y=-.019;scene.add(ground);
  const platform=new T.Mesh(new T.CylinderGeometry(.68,.7,.028,100),new T.MeshStandardMaterial({color:0xe6e4f4,metalness:.12,roughness:.67}));platform.position.y=-.016;scene.add(platform);
  const ring=new T.Mesh(new T.RingGeometry(.63,.632,128),new T.MeshBasicMaterial({color:0x7c5cff,transparent:true,opacity:.38,side:T.DoubleSide}));ring.rotation.x=-Math.PI/2;ring.position.y=.001;scene.add(ring);
  const innerRing=new T.Mesh(new T.RingGeometry(.55,.551,128),new T.MeshBasicMaterial({color:0x00c2d1,transparent:true,opacity:.22,side:T.DoubleSide}));innerRing.rotation.x=-Math.PI/2;innerRing.position.y=.001;scene.add(innerRing);
  ...
  const markerMaterial=new T.PointsMaterial({color:0x5b4a8a,size:5,sizeAttenuation:false,transparent:true,opacity:.72,depthTest:false});
  ...
  (async()=>{try{let cursor=0;await Promise.all(Array.from({length:3},async()=>{while(cursor<atlas.chunks.length){const i=cursor++;await loadChunk(i);}}));if(!disposed){ready=true;dirty=true;}}catch(e){if(!disposed)onError(e instanceof Error?e.message:t('sceneErrorLoad',lang));}})();
  ...
  // Post-processing: soft brand glow (bloom) on selection highlights and rings.
  const composer=new EffectComposer(renderer);composer.addPass(new RenderPass(scene,camera));
  const bloom=new UnrealBloomPass(new T.Vector2(el.clientWidth,el.clientHeight),.28,.7,.88);composer.addPass(bloom);
  let glowLevel=.28;
  // Region framing: aim the camera at a body band without touching user zoom.
  const frameRegion=(r:string)=>{controls.update();const dir=camera.position.clone().sub(controls.target).normalize();if(dir.lengthSq()<1e-6)dir.set(.35,.06,1).normalize();
   const bands:Record<string,[number,number]>={all:[.2,.95],head:[1.52,1.75],chest:[1.18,1.5],abdomen:[.95,1.2],pelvis:[.78,1.0],legs:[.05,.8]};
   const [lo,hi]=bands[r]??[.2,.95];const mid=(lo+hi)/2,height=Math.max(.18,(hi-lo)*1.6);
   const distance=Math.max(.25,height/(2*Math.tan(T.MathUtils.degToRad(camera.fov/2))));
   controls.target.set(0,mid,0);camera.position.copy(controls.target).addScaledVector(dir,distance);controls.update();dirty=true;};
  // Programmatic zoom: multiplies current camera distance, clamped to controls limits.
  const dolly=(factor:number)=>{const dir=camera.position.clone().sub(controls.target);const len=T.MathUtils.clamp(dir.length()*factor,controls.minDistance+.02,Math.min(12,controls.maxDistance));dir.setLength(len);camera.position.copy(controls.target).add(dir);controls.update();dirty=true;zoomCb.current?.(4/len);};
  apiRef.current={dolly,frame:frameRegion};
  const clock=new T.Clock();let lastExtent=-1,lastZoomId=-1,lastRegionId=-1,lastHq:boolean|undefined=undefined;
  const animate=()=>{
   ...
   if(s.zoomSeq&&s.zoomSeq.id!==lastZoomId){lastZoomId=s.zoomSeq.id;dolly(s.zoomSeq.dir>0?.78:1.28);}
   if(s.regionSeq&&s.regionSeq.id!==lastRegionId){lastRegionId=s.regionSeq.id;frameRegion(s.region??'all');}
   ...
   controls.enableRotate=amount<.8;controls.mouseButtons.LEFT=amount<.8?T.MOUSE.ROTATE:T.MOUSE.PAN;controls.touches.ONE=amount<.8?T.TOUCH.ROTATE:T.TOUCH.PAN;ground.visible=platform.visible=ring.visible=innerRing.visible=amount<.5&&!s.isolate;markers.visible=amount>.75||s.labels===true;controls.autoRotate=(s.rotate||s.spin===true)&&!s.isolate&&amount<.4;controls.autoRotateSpeed=.65;controls.update();if(controls.autoRotate)dirty=true;
   // X-ray: fade opaque systems to glass; glow: animate bloom strength.
   const wantGlow=s.glow===false?0.06:.28;
   if(Math.abs(wantGlow-glowLevel)>.001){glowLevel=T.MathUtils.damp(glowLevel,wantGlow,6,dt);bloom.strength=glowLevel;dirty=true;}
   const L=s.lighting;
   if(L){
    const targetExposure=.92*L.exposure;
    if(Math.abs(renderer.toneMappingExposure-targetExposure)>.004){renderer.toneMappingExposure=T.MathUtils.damp(renderer.toneMappingExposure,targetExposure,8,dt);dirty=true;}
    key.intensity=T.MathUtils.damp(key.intensity,1.55*L.key,8,dt);
    rim.intensity=T.MathUtils.damp(rim.intensity,1.15*L.rim,8,dt);
    fill.intensity=T.MathUtils.damp(fill.intensity,.5*L.ambient,8,dt);
    hemi.intensity=T.MathUtils.damp(hemi.intensity,1.05*L.ambient,8,dt);
   }
   const xray=s.xray===true;
   if(s.hq!==undefined&&s.hq!==lastHq){lastHq=s.hq;renderer.setPixelRatio(Math.min(devicePixelRatio,el.clientWidth<768?1.5:(s.hq?2:1)));dirty=true;}
   mats.forEach((mat,sys)=>{const mm=mat as unknown as {userData:{baseOpacity:number};opacity:number};const base=mm.userData.baseOpacity??1;const target=xray&&sys!=='integumentary'?.22:base;if(Math.abs(mm.opacity-target)>.004){mm.opacity=T.MathUtils.damp(mm.opacity,target,6,dt);dirty=true;}});
  ...
  const contextLost=(e:Event)=>{e.preventDefault();onError(t('sceneErrorContext',lang));};renderer.domElement.addEventListener('webglcontextlost',contextLost);
